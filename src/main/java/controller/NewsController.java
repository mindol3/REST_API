...
// import org.apache.commons.beanutils.getFilename;
import org.apache.commons.beanutils.BeanUtils;

@WebServlet("*.nhn")
// 뉴스 이미지 파일 업로드 처리를 위해 @MultipartConfig을 추가.
// 최대 파일 크기와 저장 위치를 지정.
...
public class NewsController extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// getMethod() : 첫 번째 인자로 메서드 이름, 두 번째 인자 타입을 메서드의 인자로 가지는 메서드 객체를 가져옴.
			...
			e.printStackTrace();
			ctx.log("요청 action 없음!!"); // 톰켓의 콘솔에 로드 메시지를 남기는 메서드.
